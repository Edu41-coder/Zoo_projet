...
<body>
    ...
    if (!isset($_SESSION['utilisateur'])) {
        ...
        $updateResult = $collection->findOneAndUpdate(
            ['animal_id' => $animalId],
            ['$inc' => ['nbClique' => 1]],
            ['returnDocument' => MongoDB\Operation\FindOneAndUpdate::RETURN_DOCUMENT_AFTER, 'upsert' => true, 'projection' => ['nbClique' => 1]]
        );

        if ($updateResult === null) {
            $collection->updateOne(['animal_id' => $animalId], ['$set' => ['nbClique' => 1]]);
        }

        echo "Operation successful. Current nbClique is " . ($updateResult ? $updateResult['nbClique'] : '1');
    } else {
